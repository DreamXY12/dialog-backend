...
# 1. 创建 FastAPI 主应用
#app = FastAPI(title="聊天服务", version="1.0")

# 2. 创建 Socket.IO 异步服务
sio = socketio.AsyncServer(cors_allowed_origins="*", async_mode="asgi",cors_credentials=True,allow_origin=None)

# 3. 把 Socket.IO 挂载到指定路径：/api/v1/chat
# 这是最关键的一步！和前端 io("/api/v1/chat") 完全对应
...
async def send_message(sid, data):
    ...
    if role == "patient" and rooms[room_id]["ai_enabled"]:
        asyncio.create_task(handle_ai_reply(room_id, msg))
