...
def send_message(to_number, body_text, role,is_fixed=0):
    """发送WhatsApp消息"""
    try:
        ...
        if not is_en:
            ...
            yue_body_text=body_text
        #print(f"DEBUG: Translated message: {yue_body_text}")
        ...
        return {"code":200,"data":{"message":yue_body_text,"role":role,"to_number":to_number},"error_info":""}
    except Exception as e:
        logger.error(f"Error sending message to {to_number}: {e}")
        print(f"Error sending message to {to_number}: {e}")
        return {"code": -1, "data": {"message": "接收消息失敗，請稍後重試", "role": role, "to_number": to_number},"error_info":str(e)}
...
async def extract(request: Request):
    ...
    #     "SmsSid": form_data.get("SmsSid"),
    #     "SmsMessageSid": form_data.get("SmsMessageSid"),
    #     "AccountSid": form_data.get("AccountSid"),
    #     "MessagingServiceSid": form_data.get("MessagingServiceSid"),
    #     "From": form_data.get("From"),
    #     "To": form_data.get("To"),
    #     "Body": form_data.get("Body"),
    #     "NumMedia": form_data.get("NumMedia"),
    #     "NumSegments": form_data.get("NumSegments")
    # }
...
要使用我們的服務，請輸入您的邀請碼。"""
        ...
        if question_lower in ['/cancel', 'cancel', 'quit']:
            ...
            if not is_en:
                ...
                return send_message(phone_number, "Registration has been canceled. Enter /start to begin again.", "system")
...
要使用我們的服務，請輸入您的邀請碼。"""
            ...
            if validate_invitation_code(db, code):
                ...
                set_user_state(phone_number, RegistrationState.CODE_VERIFIED)
...
我會逐一問你每個問題。我們開始吧！"""
            ...
            await handle_confirmation(phone_number, question, db)
        ...
        if not is_en:
            ...
            return send_message(phone_number, "I'm sorry, an error occurred while processing your request. Please try again.", "system")
...
async def process_health_info(phone_number, question, db):
    ...
    try:
        ...
        if collection_mode == '3':  # 一步一步模式
            ...
            if current_field:
                ...
                if field_value and validate_field(current_field, field_value):
                    ...
                    if not is_en:
                        return send_message(phone_number, f"✅ 已經取得如下信息! {get_fixed_field_translation(current_field)}: {get_fixed_response_translation(field_value.lower())}", "system")
                    else:
                ...
                # 没有当前字段，使用通用解析
                await process_general_collection(phone_number, extracted)
        ...
        if not is_en:
            ...
            return send_message(phone_number, "I'm sorry, I cannot process your information. Please try again.", "system")
...
async def process_general_collection(phone_number, extracted):
    ...
    try:
        if extracted:
            ...
            for field, value in extracted.items():
                if validate_field(field, value):
                    ...
                    update_summary.append(f"{get_fixed_field_translation(field.lower())}: {get_fixed_response_translation(value.lower())}")
        ...
        if collection_mode == '2':  # 多部分模式
            await ask_next_field_multi_part(phone_number, missing_fields)
        ...
        if is_en:
            ...
            return send_message(phone_number, "I'm sorry, an error occurred while processing your information.", "system")
    ...
    # 显示已收集和未收集的信息
        ...
        for field in ['age', 'sex', 'weight', 'height', 'family_history', 'smoking', 'alcohol']:
            if field in collected and collected[field]:
                ...
                if not is_en:
                    collected_display.append(f"✓ {get_fixed_field_translation(field_display.lower())}: {get_fixed_response_translation(collected[field].lower())}")
                else:
...
async def handle_confirmation(phone_number, response, db):
    ...
    try:
        ...
        if response_lower in ['yes', 'y', 'confirm', 'correct']:
            # 用户确认，完成注册
            await complete_registration(phone_number, db)
            ...
            if not is_en:
                ...
                return send_message(phone_number, "What do you want to change? Please tell me specifically.\nFor example: \"My weight is 70 kilograms.\"", "system")
            set_user_state(phone_number, RegistrationState.COLLECTING)
            ...
            if extracted:
                ...
                # 不理解用户的输入
                if not is_en:
                    return send_message(phone_number, "我不確定您想要更改什麼。請具體說明。\n例如：「將體重更改為70公斤」", "system")
                else:
                    return send_message(phone_number, "I'm not sure what you want to change. Please be specific.\nFor example, \"Change the weight to 70 kilograms.\"","system")
        ...
        if not is_en:
            ...
            return send_message(phone_number, "Sorry, I'm unable to process your confirmation. Please try again.", "system")
    ...
    try:
        ...
        if missing_required:
            if not is_en:
                ...
                return send_message(phone_number, f"缺少必填信息: {', '.join(missing_required_tchinese)}. 請提供這些信息.", "system")
            else:
                return send_message(phone_number, f"Missing required information: {', '.join(missing_required)}. 請提供這些信息.",
                             "system")
            set_user_state(phone_number, RegistrationState.COLLECTING)
            return
...
async def replyStatus(request: Request):
    """处理消息状态回调"""
    print("Received status callback")
    #获取消息发送的状态
