...
async def login(
    ...
    try:
        ...
        if request.user_type == "patient":
            ...
            # ======================================
            # 新增：护士登录时自动创建当日排班记录（核心逻辑）
            # ======================================
            if request.user_type == "nurse":
                # 2. 香港护士长白班时间：09:00-18:00
                import datetime
                work_start = datetime.time(hour=9, minute=0, second=0)
                work_end = datetime.time(hour=18, minute=0, second=0)
                add_nurse_work_time(db,request_user_id,work_start,work_end)
